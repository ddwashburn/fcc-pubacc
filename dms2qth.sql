CREATE OR REPLACE FUNCTION DMS2QTH(latDeg integer, latMin numeric, latSec numeric, latCard char(1),
                                   lonDeg integer, lonMin numeric, lonSec numeric, lonCard char(1),
                                   prec integer = 6)
    RETURNS varchar(8)
    AS $$
/* The QTH locator, sometimes referred to as the Maidenhead locator system is a
 * geographic coordinate system which can succinctly describe any location on
 * Earth using a short sequence of letters and numbers.
 * 
 ...
 */
    DECLARE
        ...
        lat numeric := latDeg + (latMin/60) + (latSec/3600);
        lon numeric := lonDeg + (lonMin/60) + (lonSec/3600);
    BEGIN
        -- 180 degree false easting
        IF lonCard = 'W' AND lon > 0 THEN
            lon := lon * -1;
        END IF;

        -- 90 degree false northing
        IF latCard = 'S' AND lat > 0 THEN
            lat := lat * -1;
        END IF;
